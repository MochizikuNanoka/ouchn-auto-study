...
- Update checking is based on the latest published GitHub Release tag and numeric version comparison, never branch-file inequality. Use a no-cache timestamped `GM_xmlhttpRequest` against the `releases/latest` redirect and parse `response.finalUrl`; do not use the unauthenticated GitHub REST API because its shared per-IP quota can return HTTP 403, and do not request the bare redirect because GitHub CDN may briefly cache the previous release.
